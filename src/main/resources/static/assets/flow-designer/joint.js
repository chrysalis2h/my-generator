...
joint.dia.Graph = Backbone.Model.extend({
    ...
    addCell: function(cell, opt) {
        ...
        } else if(cell.attributes.type == "uml.Class") {
            cell = new joint.shapes.uml.Class(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Interface") {
            cell = new joint.shapes.uml.Interface(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Abstract") {
            cell = new joint.shapes.uml.Abstract(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Quote") {
            cell = new joint.shapes.uml.Quote(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Generalization") {
            cell = new joint.shapes.uml.Generalization(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Association") {
            cell = new joint.shapes.uml.Association(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Composition") {
            cell = new joint.shapes.uml.Composition(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
        } else if(cell.attributes.type == "uml.Implementation") {
            cell = new joint.shapes.uml.Implementation(opt);
            if (!cell.has('z')) {
                cell.set('z', this.maxZIndex() + 1);
            }
...
joint.dia.ElementView = joint.dia.CellView.extend({
    ...
    pointerdown: function(evt, x, y) {
        ...
        var cellId = this.model.get("id");
        var cellDoms = $(paper.el).find("[model-id=" + cellId + "]");
        if (
            ...
            cellDoms.length > 0 &&
            cellDoms[0].getAttribute('class-magnet') &&
            this.can('addLinkFromMagnet') &&
            paper.options.validateMagnet.call(paper, this, evt.target)
        ) {
            ...
            var link;
            if (cellDoms[0].getAttribute('class-magnet') == 'generalization') {
                link = new joint.shapes.uml.Generalization();
            } else if (cellDoms[0].getAttribute('class-magnet') == 'association') {
                link = new joint.shapes.uml.Association();
            } else if (cellDoms[0].getAttribute('class-magnet') == 'composition') {
            ...
            var linkView = this._linkView = paper.findViewByModel(link);
            evt.target.setAttribute("magnet", "true");
            linkView.pointerdown(evt, x, y);
            evt.target.removeAttribute("magnet");
            linkView.startArrowheadMove('target', { whenNotAllowed: 'remove' });
...
joint.dia.LinkView = joint.dia.CellView.extend({
    ...
    renderArrowheadMarkers: function() {
        ...
        this._V.sourceArrowhead = V('<g></g>');
        if (this.model.attributes.type == 'uml.Association') {
            this._V.sourceArrowhead = V(markupTemplate({ end: 'source' }));
        }
...
joint.shapes.uml.Quote = joint.shapes.uml.Class.extend({
...
joint.shapes.uml.QuoteView = joint.shapes.uml.ClassView;
...
joint.shapes.uml.Generalization = joint.dia.Link.extend({
    defaults: {
        ...
        attrs: { '.marker-target': { d: 'M 10 5 L 0 10 L 10 15 z', fill: 'white' }}
    },
    arrowheadMarkup: [
        '<g>',
...
joint.shapes.uml.Implementation = joint.dia.Link.extend({
    defaults: {
        ...
        attrs: {
            '.marker-target': { d: 'M 10 5 L 0 10 L 10 15 z', fill: 'white' },
